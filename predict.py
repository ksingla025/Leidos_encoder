...
class ClassifierPredict(object):
	...
	def __init__(self,model='./models/test3',parameters='./lib/parameters.json'):
		...
		self.document_vector = self.graph.get_operation_by_name("AttentionBasedAggregator/document-vectorss")
#		self.labels_batch = self.graph.get_tensor_by_name("labels_batch:0")

		self.predict_ldcsf = self.graph.get_operation_by_name("Ldcsf-Att-Prediction-Layer/predict_ldcsf_1")
